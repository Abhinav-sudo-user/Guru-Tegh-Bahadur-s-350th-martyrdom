...
export default function Events() {
  const events = [
    [
      "Seminar competition",
      <FaChalkboardTeacher className="text-3xl text-[#E66223]" />,
    ],
    [
      "Quiz competition",
      <FaQuestionCircle className="text-3xl text-[#E66223]" />,
    ],
    ["Kirtan competition", <GiDrum className="text-3xl text-[#E66223]" />],
    ["Essay/Poem writing", <FaFileAlt className="text-3xl text-[#E66223]" />],
    [
      "Gatka Competition",
      <GiCrossedSwords className="text-3xl text-[#E66223]" />,
    ],
    ["Street Plays", <GiTheaterCurtains className="text-3xl text-[#E66223]" />],
    [
      "Tree Plantation drive",
      <FaSeedling className="text-3xl text-[#E66223]" />,
    ],
  ];

  return (
    <>
      <Navbar text="Upcoming Events" />
      <div
        style={{ fontFamily: "'Montserrat', serif" }}
        className="flex font-bold flex-col m-1 items-center text-center"
      >
        <span className="m-3 text-2xl md:w-2xl mt-7 rounded-2xl font-medium border-2 p-1 bg-[#fee5bd]/50">
          What's happening? Browse diverse events, then tap to discover details
          and sign up!
        </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-6 pb-10">
          {events.map(([name, Icon], idx) => {
            const isLastAndOdd =
              events.length % 3 === 1 && idx === events.length - 1;
            return (
              <div
                key={idx}
